...
rt_etd <- dbGetQuery(mydb, "SELECT * FROM rt_etd WHERE minutes = 'Leaving' and abbreviation = 'PITT'
                     and direction = 'North' limit 10")

rt_etd <- dbGetQuery(mydb, "SELECT * FROM rt_etd WHERE minutes = 'Leaving' and abbreviation = 'PITT'
                     and direction = 'North' limit 1000000")
...
write.csv(lag_times, "lag_times_ROCK2.csv")
