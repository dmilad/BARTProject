# - Purpose - #
#1. differing train length issue
#2. Fix the time it takes to query the db
...
#query_index being non-zero case
while (query_index > 0) {
  ...
  if (is.null(train_len)) {
    ...
    query <- paste0("SELECT * FROM rt_etd WHERE minutes = 'Leaving' and abbr = '",station_name,
                    "' and length ='", train_len[length(train_len)],
                    "' and abbreviation = 'MLBR' and direction = 'South' and date = '",new_date,"' limit 10000")
    execute_query <- dbGetQuery(mydb, query)    
  }
  ...
  if (length(train_time) >1) {
    while (train_time[length(train_time)] > train_time[(length(train_time)-1)])
    train_len[length(train_len)] <- time_stamp_fixer(as.character(strsplit(as.character(execute_query[nrow(execute_query),4]), " ")[[1]][1]))
  }
