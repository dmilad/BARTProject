...
data <- read.csv("lag_times_ROCK3.csv")
#=========================#
# - Feature Engineering - #
#=========================#
View(head(data))
...
data$l_3 <- as.numeric(data$train_length==3)
...
x <- as.matrix(data[,c(7:29,31:ncol(data))], sparse = T)
y <- data$lag
...
#making this plot prettier for Elizabeth to pop into the UI

x_prime <- as.matrix(data[,c(7:16,ncol(data))], sparse = T)
corr_matrix_prime <- cor(x_prime)
corrplot(corr_matrix_prime, method = "circle")

x_prime2 <- as.matrix(data[,c(17:29,31:ncol(data))], sparse = T)
corr_matrix_prime2 <- cor(x_prime2)
corrplot(corr_matrix_prime2, method = "circle")

x_prime3 <- as.matrix(data[,c((ncol(data)-1):ncol(data))], sparse = T)
corr_matrix_prime3 <- cor(x_prime3)
corrplot(corr_matrix_prime3, method = "circle")
#========#
# - ML - #
#========#
...
print(minimum_lambda) #0.01393974

#Print out the coefficients
coefficients <- coef(cv_fit, s = "lambda.min")
coefficients <- as.numeric(coefficients)
coefficients <- data.frame(c("intercept",colnames(data[,c(7:29,31:ncol(data))])),coefficients)
View(coefficients)

write.csv(coefficients, "lag_time_coeffs2_L2.csv")
...
minimum_lambda <- cv_fit$lambda.min
print(minimum_lambda) #0.0005248223
...
write.csv(coefficients, "lag_time_coeffs2_L1.csv")

# - Interesting Thought - #
#Are lag times more for trains farther down in the sequence?
data2 <- data

station_list <- c('PITT', 'NCON', 'CONC', 'PHIL', 'WCRK', 'LAFY', 'ORIN', 'ROCK', 'MCAR', '19TH', '12TH',
                  'WOAK', 'EMBR', 'MONT', 'POWL', 'CIVC', '16TH', '24TH', 'GLEN', 'BALB',
                  'SBRN', 'SFIA', 'MLBR') 
station_list <- rev(station_list)

station_mappers <- data.frame(station_list, 1:length(station_list))
colnames(station_mappers) <- c("station", "stops_away_from_start")

data2 <- merge(data2, station_mappers, by="station")

x <- as.matrix(data2[,7:ncol(data2)], sparse = T)
y <- data2$lag
...
plot(cv_fit, main = "L2 Regularization")
minimum_lambda <- cv_fit$lambda.min
print(minimum_lambda) #0.06938612

#Print out the coefficients
coefficients <- coef(cv_fit, s = "lambda.min")
coefficients <- as.numeric(coefficients)
coefficients <- data.frame(c("intercept",colnames(data2[,7:ncol(data2)])),coefficients)
View(coefficients)

summary(lm(lag~stops_away_from_start, data2))
